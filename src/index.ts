...
try {
  ...
  if (import.meta.main) {
    ...
    const app = new Elysia({
      ...
      detail: {
        description: `${constants.server.name} Server API`,
        tags: ["Root"],
      },
    })
    ...
    app
      ...
      .use(
        cron({
          ...
          maxRuns: undefined,
          run() {
            AuthService.clearExpiredEmailVerificationCodes().then(
              (res: any) => {
        ...
        cron({
          ...
          run() {
            ...
            NotificationService.manager
              ...
              .then((count: number) => {
                console.log(
                  `Synced ${count} websocket connections to database`,
                );
              });
          },
        }),
      ...
      // ROUTES
      .get("/", () => {
        return { message: "Server running, select version" };
      })
      .get("/health", ({ status, set }) =>
        status(200, { message: "All Systems GO" })
          .ws("/ws", {
