...
export abstract class CouponService {
  ...
  static async getCouponByCode(couponCode: string, profileId?: string) {
  ...
  static async getCouponsByUserProfileId(profileId: string) {
  ...
  // Retrieve a User's Coupons by their Profile ID [ADMIN | STAFF]
  static async getAllCouponsAsAdmin(params?: { isActive: boolean }) {
    return db.coupon.findMany({
      where: {
  ...
  // Use a coupon
  static async useCoupon(profileId: string, code: string): Promise<Coupon> {
    try {
