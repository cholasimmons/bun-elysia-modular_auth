...
export abstract class UsersService {
  static async getAll(isActive?: boolean, profiles?: boolean) {
    try {
  ...
  static async getUser(
    userId: string,
    opts?: { profile?: boolean; isActive?: boolean },
  ): Promise<PrismaUserWithOptionalProfile> {
    try {
      let user: PrismaUserWithOptionalProfile | null =
        await redisGet<PrismaUserWithOptionalProfile | null>(`user:${userId}`);
  ...
  static async createUserProfile(data: any): Promise<ProfileWithSafeUserModel> {
    const {
      firstname,
      lastname,
      documentId,
    ...
    try {
      ...
      const userWithProfile: PrismaUserWithProfile | null =
      ...
      if (!userWithProfile?.profile) {
        throw new InternalServerError(
          "Could not update User with new User Profile",
        );
      }
      ...
      await redisSet(
        `profile:user:${userWithProfile.id}`,
        returnSafeUserProfile,
        3,
      );
  ...
  modifyRoles = {
    add(
      roles: Role[],
      newRoles: Role | Role[],
      replaceRoles?: Role | Role[],
    ): Role[] {
      // Normalize to array
      const newRolesArray = Array.isArray(newRoles) ? newRoles : [newRoles];
      const replaceRolesArray = replaceRoles
        ? Array.isArray(replaceRoles)
          ? replaceRoles
          : [replaceRoles]
        : [];
      ...
      roles.push(
        ...newRolesArray.filter((newRole) => !roles.includes(newRole)),
      );
    ...
    remove(roles: Role[], removeRoles: Role | Role[]): Role[] {
      // Normalize to array
      const removeRolesArray = Array.isArray(removeRoles)
        ? removeRoles
        : [removeRoles];
  ...
  static sanitizeUserObject(user: User): SafeUser {
  ...
  static async getProfileByUserId(
    ...
    try {
      let cachedProfile: ProfileWithPartialUser | null =
        await redisGet<ProfileWithPartialUser>(`profile:user:${userId}`);
      ...
      if (!cachedProfile) {
        cachedProfile = await db.profile.findUnique({
          where: { userId: userId },
          include: {
            user: includeUser,
        ...
        if (!cachedProfile)
          throw new NotFoundError(`Profile for User: ${userId} unavailable.`);
  ...
  addUserRoles = async ({ set, params, body }: any) => {
    ...
    try {
      ...
      const roles = this.modifyRoles.add(user.roles, newRoles, oldRoles);
      ...
      const cleanUser: SafeUser = UsersService.sanitizeUserObject(dbUser);

      return {
        data: cleanUser,
        message: `Updated User Roles ${cleanUser.roles}`,
      };
    } catch (error) {
      // console.error(error);
  ...
  // TODO: work on these
  static async addPrefs(userId: string, newPrefs: string) {
    const user: User | null = await db.user.update({
      where: { id: userId },
      data: { prefs: JSON.stringify(newPrefs) },
    });
  }
  static async removePrefs(userId: string, prefs: string) {
    const user: User | null = await db.user.update({
      where: { id: userId },
      data: { prefs: JSON.stringify(prefs) },
    });
  }
  static async replacePrefs(userId: string, newPrefs: string) {
    const user: User | null = await db.user.update({
      where: { id: userId },
      data: { prefs: JSON.stringify(newPrefs) },
    });
  ...
  static async getCachedUser(userId: string): Promise<SafeUser> {
    ...
    if (!user) {
      ...
      if (!dbUser)
        throw new NotFoundError(
          "User not found",
          404,
          "Could not find User in database or cache",
        ); // Return null if user doesn't exist in the database

      user = UsersService.sanitizeUserObject(dbUser);
