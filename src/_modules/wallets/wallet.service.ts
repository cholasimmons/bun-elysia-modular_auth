...
export abstract class WalletService {
  ...
  static async create(
    ...
    query?: { initialBalance: number; currency: Currency },
  ): Promise<Wallet | null> {
    try {
      ...
      const wallet: Wallet | null = await db.wallet.create({
        data: {
          ...
          balance: query?.initialBalance ?? 0.0,
        },
  ...
  static async getWalletByID(
    userProfileId: string,
    query?: { transactions?: boolean; profile?: boolean },
  ) {
      ...
      return db.$transaction(
        async (tx) => {
          ...
          if (discountCode) {
            ...
            discountObj = await tx.coupon.findFirst({
          ...
          if (
            discountObj.usedBy.length >= discountObj.maxUses ||
            now >= (discountObj.expiresAt ?? now)
          ) {
          ...
          const discountValue =
            discountObj.discountType === DiscountType.FLAT
              ? discount
              : bill * discount;
          ...
          if (Number(totalBill) > Number(wallet.balance)) {
            throw {
              balance: `Insufficient balance in wallet. ${wallet.currency}${wallet.balance}`,
            };
          }
