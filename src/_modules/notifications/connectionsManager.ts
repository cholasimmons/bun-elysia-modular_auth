import { WSConnections } from "@generated/prisma/client";
...
export class ConnectionManager {
  ...
  async addConnection(ws: ElysiaWS, userId: string, connectionId?: string) {
    ...
    ws.send({
      message: `Welcome to ${constants.server.name} ${userId}`,
      data: connId,
    });
  }
  ...
  async removeConnection(connectionId: string): Promise<void> {
    ...
    if (!userId) {
      console.warn(
        `Connection ID ${connectionId} not found in userConnections reverse map.`,
      );
      return;
    }
  ...
  async recoverConnections() {
    ...
    for (const userKey of allUserKeys) {
      ...
      const connectionId: string | null =
        await this.getCachedConnection(userId);
  ...
  async getCachedConnection(userId: string): Promise<string | null> {
    let connection: string | null = await redisGet(
