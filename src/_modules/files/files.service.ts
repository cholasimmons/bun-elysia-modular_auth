...
import {
  ...
  getFiles,
  storeBuffer,
} from "~modules/files/minio.controller";
...
const BUCKET_FILES: string = process.env.BUCKET_FILES || "hello-files";
const WATERMARK_SQUARE: BunFile = Bun.file(
  "./public/images/logos/elysia_icon.webp",
);
...
const imageMainQuality: number = Number(process.env.IMAGE_QUALITY ?? 75);
const imageThumbQuality: number = Number(process.env.THUMBNAIL_QUALITY ?? 40);

export abstract class FilesService {
  // Check if selected bucket exists
  static pingBucket = async (bucket: BucketType): Promise<boolean> => {
    try {
  ...
  static pingBucketAndCreate = async (bucket: BucketType): Promise<boolean> => {
    try {
  ...
  static listAllFiles = async (bucket: BucketType) => {
    try {
  ...
  static getFileByName = async (filename: string, bucket: BucketType) => {
    try {
  ...
  static getFilesByUserId = async (userId: string) => {
  ...
  static uploadFile = async (
    ...
    try {
      ...
      const existingFile = await db.fileUpload.findFirst({
        where: { hash: fileHash },
      });
      if (existingFile) {
      ...
      await storeBuffer(buffer, bucketName, {
        ...metadata,
        name: uniqueFileName,
      });
      ...
      const processedFiles = await Promise.all(
        files.map(async (file, index) => {
          const metadata = {
            ...
            size: file.size,
            type: file.type,
            "X-Amz-Meta-UploaderUserId": userProfileId,
            "X-Amz-Meta-CreatedDate": createdDate.toISOString(),
          };
  ...
  static uploadPhoto = async (
    ...
    const generatedName = fileName ? fileName : dateToFilename(createdDate);
    let uniqueFileName = `${generatedName}_${createdDate.getTime()}${fileExtension ? "." + fileExtension : ""}`;

    try {
      const metadata: IImageUpload = {
        name: file.name,
        size: file.size,
        type: file.type,
        "X-Amz-Meta-UploaderUserId": userId,
        "X-Amz-Meta-CreatedDate": createdDate.toISOString(),
      };
      ...
      const resizedBuffer = await sharp(arrayBuffer)
        ...
        .resize(
          constants.images.main.width || 1280,
          constants.images.main.height || 1280,
        )
        .toFormat(imageFormat, {
          quality: imageMainQuality || constants.images.main.quality || 78,
        })
        .toBuffer();
      ...
      if (hasBlur) {
        blurBuffer = await sharp(blurBuffer)
          .blur(constants.images.blurAmount || 9)
          .toFormat(imageFormat, {
            quality: imageMainQuality || constants.images.main.quality || 78,
          })
          .toBuffer();
      ...
      const thumbImage = await sharp(finalBuffer)
        .resize(
          constants.images.thumbnail.width || 196,
          constants.images.thumbnail.height || 196,
        )
        .toFormat(imageFormat, {
          quality:
            imageThumbQuality || constants.images.thumbnail.quality || 48,
        })
        .toBuffer();
      // Upload images to MinIO
      await Promise.all([
        storeBuffer(finalBuffer, bucketName, {
          ...metadata,
          name: "main_" + uniqueFileName,
          "X-Amz-Meta-Mode": UploadMode.MAIN,
        }),
        hasBlur
          ? storeBuffer(blurBuffer, bucketName, {
              ...metadata,
              name: "blur_" + uniqueFileName,
              "X-Amz-Meta-Mode": UploadMode.BLUR,
            })
          : null,
        storeBuffer(thumbImage, bucketName, {
          ...metadata,
          name: "thumb_" + uniqueFileName,
          "X-Amz-Meta-Mode": UploadMode.THUMB,
        }),
      ]);
      ...
      const processedImages = await Promise.all(
        files.map(async (file, index) => {
          ...
          if (hasBlur) {
            blurBuffer = await sharp(blurBuffer)
              .resize(
                constants.images.main.width || 640,
                constants.images.main.height || 640,
              )
              .blur(constants.images.blurAmount)
              .toFormat(imageFormat, { quality: imageMainQuality })
              .toBuffer();
          }
          ...
          // Upload images to MinIO
          await Promise.all([
            storeBuffer(mainImage, bucketName, {
              ...metadata,
              name: "main_" + uniqueFileName,
              "X-Amz-Meta-Mode": UploadMode.MAIN,
            }),
