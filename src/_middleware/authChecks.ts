...
export const headerCheck = ({ request: { headers }, authMethod }: any) => {
  if (!authMethod) {
    const method: string | null = headers.get(
      constants.auth.method ?? "X-Client-Type",
    );
...
export const checkAuth = async ({
  ...
  if (authMethod === "Cookie") {
    ...
    throw new AuthenticationError(
      `${authMethod} unsupported authentication method`,
    );
  }
...
export const checkIsStaff = async ({ user, error }: any) => {
  ...
  if (
    !roles.some((role: any) => [Role.SUPERVISOR, Role.SUPPORT].includes(role))
  ) {
    throw new AuthorizationError("Access denied. Insufficient privileges");
  }
};
...
export const checkForProfile = async ({ user, session, error }: any) => {
  ...
  if (!user || !isActive) {
    throw new AuthenticationError(
      `User Account disabled. ${user.isComment ?? ""}`,
    );
  }
